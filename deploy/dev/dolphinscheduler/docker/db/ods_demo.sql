...
select '2024-06-01 00:00:00' as start_dt, '9999-12-31 00:00:00' as end_dt, `id` AS `order_id`, `price` from `demo`.`orders`;
...
FROM orders_zip oz
WHERE EXISTS (
SELECT 1 FROM `demo`.`table_row_change_log` cl WHERE cl.dt = '2024-06-01' AND cl.table_name = 'orders' AND cl.pk_1 = oz.order_id AND cl.op IN ('d', 'u')
);
...
FROM `demo`.`orders` o
WHERE EXISTS (
SELECT 1 FROM `demo`.`table_row_change_log` cl WHERE cl.dt = '2024-06-01' AND cl.table_name = 'orders' AND cl.pk_1 = o.id AND cl.op IN ('c', 'u')
);
...
FROM orders_zip oz
WHERE EXISTS (
SELECT 1 FROM `demo`.`table_row_change_log` cl WHERE cl.dt = '2024-06-02' AND cl.table_name = 'orders' AND cl.pk_1 = oz.order_id AND cl.op IN ('d', 'u')
);
...
FROM `demo`.`orders` o
WHERE EXISTS (
SELECT 1 FROM `demo`.`table_row_change_log` cl WHERE cl.dt = '2024-06-02' AND cl.table_name = 'orders' AND cl.pk_1 = o.id AND cl.op IN ('c', 'u')
);
